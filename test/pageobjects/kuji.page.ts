...
class KujiPage extends Page {
  ...
  async handleClickKujiElementIndex(index: number): Promise<boolean> {
    ...
    console.log("url after click kuji: " + url);
  ...
  async handleClickEntry() {
    try {
      ...
      try {
        ...
        let isDisplayedEntry = await this.isDisplayedEntry();
        let loopCount = 0;
        while (!isDisplayedEntry && loopCount < 10) {
          // scroll downn 5 %
          await browser.execute(
            "window.scrollBy(0, window.innerHeight * 0.05)",
          );
          await browser.pause(1000);
          isDisplayedEntry = await this.isDisplayedEntry();
          loopCount++;
        }
      }
      ...
      await browser.waitUntil(
        async () => {
          try {
            ...
            if (
              url.indexOf(config.KUJI_HOME_PAGE) == -1 ||
              diffMs >= config.DEFAULT_KUJI_PAGE_TIMEOUT
            ) {
      ...
      console.log("handleClickEntry error: " + error);
    }
  }

  async handleProcessAfterClickKuji(
    index: number,
